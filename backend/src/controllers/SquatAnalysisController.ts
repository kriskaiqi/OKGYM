...
export class SquatAnalysisController {
  private squatAnalyzerService: BaseExerciseAnalyzerService;

  constructor() {
    // Get the squat analyzer from the factory
    this.squatAnalyzerService = ExerciseAnalyzerFactory.getInstance().getAnalyzer('squat');
    
    // Initialize service on startup to avoid cold start delay
    this.squatAnalyzerService.initialize().catch(err => {
   ...
   */
  public async analyzeFrame(data: any): Promise<SquatAnalysisResponse> {
    try {
      ...
      if (Math.random() < 0.1) {
        logger.info(`Processing ${poseLandmarks.length} landmarks for analysis (pre-mirrored: ${isMirrored ? 'yes' : 'no'})`);
      }
      ...
      if (Math.random() < 0.1) {
        logger.info(`Starting pose analysis at ${new Date().toISOString()}`);
      }
      
      const result = await this.squatAnalyzerService.analyzePoseData(poseData);
      ...
      if (processingTime > 100 || (result.success && result.result?.repCount && result.result.repCount > 0)) {
        logger.info(`Completed pose analysis in ${processingTime}ms: success=${result.success}, has result=${!!result.result}`);
      }
      ...
      if (result.success && result.result) {
        ...
        if ((result.result.repCount && result.result.repCount > 0) || Math.random() < 0.1) {
          logger.info(`Sending analysis result: stage=${result.result.stage}, score=${result.result.formScore}, reps=${result.result.repCount}`);
        }
      }
  ...
  public async resetRepCounter(): Promise<boolean> {
    try {
      logger.info('RESET_DEBUG: SquatAnalysisController - Resetting repetition counter');
      const result = await this.squatAnalyzerService.resetRepCounter();
