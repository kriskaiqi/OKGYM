...
import { LungeAnalysisResponse } from '../types/lunge';
...
export class LungeAnalysisController {
  private lungeAnalyzerService: BaseExerciseAnalyzerService;

  constructor() {
    // Get the lunge analyzer from the factory
    this.lungeAnalyzerService = ExerciseAnalyzerFactory.getInstance().getAnalyzer('lunge');
    ...
    logger.info('Initializing LungeAnalysisController and its analyzer service');
    this.lungeAnalyzerService.initialize()
      .then(() => {
        logger.info('LungeAnalysisController initialized successfully');
      })
      .catch(err => {
        logger.warn('Non-critical error during LungeAnalysisController initialization:', err);
      });
  }

  /**
   * Analyze a single frame for lunge form
   */
  public async analyzeFrame(data: any): Promise<LungeAnalysisResponse> {
    try {
      ...
      if (Math.random() < 0.1) {
        logger.info(`Processing ${poseLandmarks.length} landmarks for lunge analysis (pre-mirrored: ${isMirrored ? 'yes' : 'no'})`);
      }
      ...
      if (Math.random() < 0.1) {
        logger.info(`Starting lunge pose analysis at ${new Date().toISOString()}`);
      }
      
      const result = await this.lungeAnalyzerService.analyzePoseData(poseData);
      ...
      if (processingTime > 100 || (result.success && result.result?.repCount && result.result.repCount > 0)) {
        logger.info(`Completed lunge pose analysis in ${processingTime}ms: success=${result.success}, has result=${!!result.result}`);
      }
      ...
      if (result.success && result.result) {
        ...
        if ((result.result.repCount && result.result.repCount > 0) || Math.random() < 0.1) {
          logger.info(`Sending lunge analysis result: stage=${result.result.stage}, score=${result.result.formScore}, reps=${result.result.repCount}`);
        }
      }
      
      return result;
    } catch (error) {
      logger.error('Error in lunge analyzeFrame:', error);
      return {
        success: false,
        error: {
          type: 'ANALYSIS_ERROR',
  ...
  public async resetRepCounter(): Promise<boolean> {
    try {
      logger.info('RESET_DEBUG: LungeAnalysisController - Resetting repetition counter');
      const result = await this.lungeAnalyzerService.resetRepCounter();
