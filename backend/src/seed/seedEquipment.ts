...
/**
 * Seed the database with common exercise equipment
 * Total: 20 equipment items as specified in the seeding plan
 */
export async function seedEquipment(): Promise<void> {
  try {
    ...
    if (existingCount > 0) {
      logger.info(`Database already has ${existingCount} equipment items. Skipping seed.`);
      return;
    }
    
    // Get all exercise categories to associate with equipment
    const allCategories = await categoryRepository.find();
    
    if (allCategories.length === 0) {
      logger.warn('No exercise categories found. Equipment will be created without category associations.');
    }
    
    // Filter categories by type for easy reference
    const muscleGroupCategories = allCategories.filter(cat => cat.type === CategoryType.MUSCLE_GROUP);
    const equipmentTypeCategories = allCategories.filter(cat => cat.type === CategoryType.EQUIPMENT);
    const specialCategories = allCategories.filter(cat => cat.type === CategoryType.SPECIAL);
    const movementPatternCategories = allCategories.filter(cat => cat.type === CategoryType.MOVEMENT_PATTERN);
    
    // Get category by name helper function
    const getCategoryByName = (name: string): ExerciseCategory | undefined => {
      return allCategories.find(cat => cat.name.toLowerCase() === name.toLowerCase());
    };
    
    // Create sample equipment with enhanced metadata
    const equipmentData = [
      // Free weights
      {
        name: 'Dumbbells',
        description: 'Variable weight handheld weights for a wide range of exercises.',
        category: EquipmentCategory.DUMBBELLS,
        difficulty: Difficulty.BEGINNER,
        costTier: CostTier.MID_RANGE,
      ...
      },
      {
        ...
        costTier: CostTier.MID_RANGE,
        spaceRequired: SpaceRequirement.MEDIUM,
      ...
      },
      {
        ...
        category: EquipmentCategory.KETTLEBELLS,
        difficulty: Difficulty.INTERMEDIATE,
        costTier: CostTier.MID_RANGE,
        spaceRequired: SpaceRequirement.SMALL,
      ...
      // Resistance equipment
      {
        ...
        category: EquipmentCategory.RESISTANCE_BANDS,
        difficulty: Difficulty.BEGINNER,
        costTier: CostTier.BUDGET,
        spaceRequired: SpaceRequirement.MINIMAL,
      ...
      },
      {
        ...
        spaceRequired: SpaceRequirement.LARGE,
    ...
    for (const item of equipmentData) {
      ...
      // Media relationships will be handled separately once we have media assets
      
      // Type assertion to handle specifications
      equipment.specifications = {} as any;
      if (item.specifications) {
        // Just copy properties that aren't null and aren't weight, dimensions, or maxUserWeight
        Object.entries(item.specifications).forEach(([key, value]) => {
          if (value !== null){
            // Convert string 'adjustable' to boolean
            if (key === 'adjustable' && typeof value === 'string') {
              (equipment.specifications as any)[key] = 
                value.includes('adjustable') || value.includes('Available');
            } else {
              (equipment.specifications as any)[key] = value;
            }
          }
        });
      }
      ...
      if (muscleGroupCategories.length > 0) {
        ...
        for (const muscleGroupName of item.targetMuscleGroups) {
          const category = muscleGroupCategories.find(cat => 
            cat.name.toLowerCase() === muscleGroupName.toLowerCase()
          );
    ...
    logger.info(`Successfully seeded ${equipmentData.length} equipment items`);
  } catch (error) {
