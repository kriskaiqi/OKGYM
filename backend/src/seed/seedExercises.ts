...
/**
 * Generate realistic exercise stats data
 * @returns Random but realistic exercise statistics
 */
const generateExerciseStats = () => {
  // Generate random rating data
  const ratingCount = Math.floor(Math.random() * 200) + 50; // 50-250 ratings
  const ratingValue = parseFloat((Math.random() * 2 + 3).toFixed(1)); // 3.0-5.0 rating
  
  // Generate rating distribution (ensure numbers add up to ratingCount)
  const star5 = Math.floor(ratingCount * (0.4 + Math.random() * 0.3)); // 40-70% 5-star
  const star4 = Math.floor(ratingCount * (0.2 + Math.random() * 0.2)); // 20-40% 4-star
  const star3 = Math.floor(ratingCount * (0.05 + Math.random() * 0.15)); // 5-20% 3-star
  const star2 = Math.floor(ratingCount * (0.02 + Math.random() * 0.08)); // 2-10% 2-star
  let star1 = ratingCount - (star5 + star4 + star3 + star2); // Remainder to ensure sum = ratingCount
  star1 = Math.max(0, star1); // Ensure non-negative
  
  // Generate random calorie data based on exercise type
  const caloriesAvg = Math.floor(Math.random() * 150) + 50; // 50-200 avg calories
  const caloriesMin = Math.floor(caloriesAvg * (0.5 + Math.random() * 0.3)); // 50-80% of avg
  const caloriesMax = Math.floor(caloriesAvg * (1.2 + Math.random() * 0.8)); // 120-200% of avg
  
  // Generate random duration data
  const durationAvg = Math.floor(Math.random() * 300) + 120; // 2-7 minutes avg (in seconds)
  const durationMin = Math.floor(durationAvg * (0.5 + Math.random() * 0.3)); // 50-80% of avg
  const durationMax = Math.floor(durationAvg * (1.2 + Math.random() * 0.8)); // 120-200% of avg
  
  // Generate completion data
  const completionRate = parseFloat((Math.random() * 0.3 + 0.65).toFixed(2)); // 65-95% completion rate
  const completionTotal = Math.floor(Math.random() * 1000) + 200; // 200-1200 attempts
  const completionSuccessful = Math.floor(completionTotal * completionRate);
  
  // Generate popularity data
  const popularityScore = Math.floor(Math.random() * 80) + 20; // 20-100 popularity score
...
export async function seedExercises(): Promise<void> {
  try {
    ...
    if (existingCount > 0) {
      logger.info(`Database already has ${existingCount} exercises. Checking for duplicates.`);
    }
    ...
    if (muscleGroupCategories.length === 0) {
      logger.warn('No muscle group categories found. Exercises will be created without muscle group associations.');
    }
    
    // Get equipment
    const allEquipment = await equipmentRepository.find();
    
    // Find equipment by name function
    const findEquipmentByName = (name: string) => {
    ...
    // Helper function to convert string arrays to MuscleGroup objects
    const createMuscleGroup = (primary: string[], secondary: string[] = []): MuscleGroup => {
      return {
        primary,
        secondary
      } as MuscleGroup;
    ...
    const exercisesData = [
      ...
      // Implemented: 8/8
      {
        ...
        trackingFeatures: [TrackingFeature.COUNT, TrackingFeature.FORM],
...

      {
        ...
        measurementType: MeasurementType.REPS,
        types: [ExerciseType.CORE, ExerciseType.STRENGTH_ISOLATION],
        level: Difficulty.BEGINNER,
      ...
      },
      {
        ...
        trackingFeatures: [TrackingFeature.COUNT],
        equipmentNames: ['Bodyweight'],
        form: {
          muscles: {
            primary: ['CORE', 'OBLIQUES'],
...

      {
        ...
        measurementType: MeasurementType.REPS,
        types: [ExerciseType.CARDIO, ExerciseType.CORE],
        level: Difficulty.BEGINNER,
        ...
        trackingFeatures: [TrackingFeature.TEMPO, TrackingFeature.TEMPO],
        equipmentNames: ['Bodyweight'],
        form: {
          muscles: {
      ...
      
      {
        ...
        trackingFeatures: [TrackingFeature.FORM, TrackingFeature.RANGE],
      ...
      },
      {
        ...
        measurementType: MeasurementType.DURATION,
        types: [ExerciseType.STRENGTH_COMPOUND, ExerciseType.STRENGTH_ISOLATION],
        level: Difficulty.INTERMEDIATE,
      ...
      },
      {
        ...
        measurementType: MeasurementType.REPS,
        types: [ExerciseType.STRENGTH_COMPOUND, ExerciseType.STRENGTH_COMPOUND],
        level: Difficulty.INTERMEDIATE,
        movementPattern: MovementPattern.LUNGE,
        targetMuscleGroups: ['QUADRICEPS', 'HAMSTRINGS', 'GLUTES'],
      ...
      },
      {
        ...
        measurementType: MeasurementType.REPS,
        types: [ExerciseType.CARDIO, ExerciseType.STRENGTH_COMPOUND],
        level: Difficulty.INTERMEDIATE,
        movementPattern: MovementPattern.PUSH,
    ...
    // Add Combined Upper Body exercises
    exercisesData.push(
      {
    ...
    exercisesData.push(
      ...
      },
      {
        ...
        measurementType: MeasurementType.REPS,
        types: [ExerciseType.CARDIO, ExerciseType.HIIT],
        level: Difficulty.BEGINNER,
        movementPattern: MovementPattern.LOCOMOTION,
    ...
    // Add any additional non-duplicate exercises below
    ...
    exercisesData.push(
      ...
      },
      {
        ...
        trackingFeatures: [TrackingFeature.FORM, TrackingFeature.BALANCE],
        equipmentNames: ['Bodyweight', 'dumbbells'],
        form: {
          muscles: {
    ...
    // Add Lower Body exercises - Part 1
    exercisesData.push(
      {
    ...
    // Add Upper Body exercises - Part 2
    exercisesData.push(
      {
        name: 'Arnold Press',
      ...
      },
      {
        ...
        measurementType: MeasurementType.WEIGHT,
        types: [ExerciseType.STRENGTH_COMPOUND, ExerciseType.POWER],
        level: Difficulty.INTERMEDIATE,
        movementPattern: MovementPattern.HINGE,
        targetMuscleGroups: ['GLUTES', 'HAMSTRINGS'],
        synergistMuscleGroups: ['LOWER_BACK', 'CORE', 'SHOULDERS', 'QUADS'],
        trackingFeatures: [TrackingFeature.FORM, TrackingFeature.POWER],
    ...
    exercisesData.push(
      ...
      },
      {
        ...
        measurementType: MeasurementType.REPS,
        types: [ExerciseType.STRENGTH_COMPOUND, ExerciseType.CORE],
        level: Difficulty.INTERMEDIATE,
        movementPattern: MovementPattern.CORE,
    ...
    // Add Core exercises - Part 2 and Cardio exercises
    exercisesData.push(
      {
        name: 'Toe Touches',
      ...
      },
      {
        ...
        trackingFeatures: [TrackingFeature.FORM, TrackingFeature.TEMPO],
        equipmentNames: ['Bodyweight', 'exercise_mat'],
        form: {
          muscles: {
            primary: ['ABS', 'CORE'],
      ...
      },
      {
        ...
        trackingFeatures: [TrackingFeature.COUNT, TrackingFeature.TEMPO],
      ...
      },
      {
        ...
        measurementType: MeasurementType.REPS,
        types: [ExerciseType.PLYOMETRIC, ExerciseType.POWER],
        level: Difficulty.INTERMEDIATE,
        movementPattern: MovementPattern.SQUAT,
        targetMuscleGroups: ['QUADS', 'GLUTES', 'HAMSTRINGS'],
        synergistMuscleGroups: ['CALVES', 'CORE', 'LOWER_BACK'],
        trackingFeatures: [TrackingFeature.POWER, TrackingFeature.FORM],
    ...
    for (const exerciseData of exercisesData) {
      // Check for exact name match after basic normalization
      ...
      if (existingSimilar) {
        logger.info(`Skipping exercise "${exerciseData.name}" - exact match already exists`);
        skippedCount++;
        continue;
      }
      ...
      // Convert string arrays to proper MuscleGroup enum values
      if (exerciseData.targetMuscleGroups && exerciseData.targetMuscleGroups.length > 0) {
      exercise.targetMuscleGroups = exerciseData.targetMuscleGroups as unknown as MuscleGroup[];
      } else {
        exercise.targetMuscleGroups = [];
      }
      
      if (exerciseData.synergistMuscleGroups && exerciseData.synergistMuscleGroups.length > 0) {
      exercise.synergistMuscleGroups = exerciseData.synergistMuscleGroups as unknown as MuscleGroup[];
      } else {
        exercise.synergistMuscleGroups = [];
      }
      
      // Ensure form data is preserved correctly
      exercise.form = exerciseData.form;
      
      // Find and set muscle group categories
      if (muscleGroupCategories.length > 0) {
        exercise.categories = muscleGroupCategories.filter(category => 
          exerciseData.targetMuscleGroups.includes(category.name.toUpperCase())
        );
      }
      
      // Find and set equipment
      ...
      // When creating/updating exercises, add stats field
    ...
    logger.info(`Successfully seeded ${addedCount} exercises, skipped ${skippedCount} duplicates`);
  } catch (error) {
...
/**
 * Updates stats for all existing exercises
 */
export async function updateExerciseStats(): Promise<void> {
  try {
    const exerciseRepository = AppDataSource.getRepository(Exercise);
    
    // Get all exercises
    const exercises = await exerciseRepository.find();
    logger.info(`Found ${exercises.length} exercises to update stats for.`);
    ...
    for (const exercise of exercises) {
      ...
      if (updatedCount % 10 === 0) {
        logger.info(`Updated stats for ${updatedCount}/${exercises.length} exercises`);
      }
    }
    
    logger.info(`Successfully updated stats for all ${updatedCount} exercises`);
  } catch (error) {
