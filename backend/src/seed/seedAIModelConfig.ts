import { AIModelConfig, ModelType, ModelStatus } from '../models/AIModelConfig';
import { AppDataSource } from '../data-source';
import logger from '../utils/logger';

/**
 * Seed the database with AI model configurations for exercise form detection and tracking
 */
export async function seedAIModelConfig(): Promise<void> {
  try {
    const modelConfigRepository = AppDataSource.getRepository(AIModelConfig);
    
    // Check if we already have model configurations
    const existingCount = await modelConfigRepository.count();
    if (existingCount > 0) {
      logger.info(`Database already has ${existingCount} AI model configurations. Skipping seed.`);
      return;
    }
    
    // Create AI model configuration data
    const modelConfigsData = [
      ...
      // Movement Tracking Models
      {
        ...
        type: ModelType.MOVEMENT_TRACKING,
        version: '1.0.0',
        status: ModelStatus.ACTIVE,
    ...
    for (const configData of modelConfigsData) {
      const modelConfig = new AIModelConfig();
      Object.assign(modelConfig, configData);
      await modelConfigRepository.save(modelConfig);
    }
    
    logger.info(`Successfully seeded ${modelConfigsData.length} AI model configurations`);
  } catch (error) {
    logger.error('Error seeding AI model configurations:', error);
    throw error;
  }
} 