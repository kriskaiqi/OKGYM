...
async function fixAchievementTable() {
  ...
  try {
    ...
    // Update any invalid UUIDs to NULL directly - using a try/catch to handle any errors
    logger.info("Setting any problematic UUID values to NULL...");
    
    try {
      // Try to set all workout_session_id values to NULL when they're problematic
      await queryRunner.query(`
        UPDATE achievement 
        SET workout_session_id = NULL 
        WHERE workout_session_id::text NOT SIMILAR TO '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}'
      `);
      logger.info("Successfully updated problematic UUIDs to NULL");
    } catch (error) {
      logger.warn("Could not update using pattern matching. Trying direct null update for empty strings.");
      
      try {
        // Fallback: directly set all empty strings to NULL to fix immediate issue
        const result = await queryRunner.query(`
          UPDATE achievement 
          SET workout_session_id = NULL 
          WHERE workout_session_id IS NOT NULL
        `);
        logger.info(`Updated ${result.length > 0 ? result[0].count : 0} records to NULL`);
      } catch (e) {
        logger.error("Failed to update UUID values:", e);
      }
    }
