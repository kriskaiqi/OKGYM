...
async function fixColumnTypeMismatch() {
    ...
    try {
        ...
        try {
            // Check if the workout_plan_id column exists and its current type
            ...
            if (columnInfo.length > 0) {
                ...
                if (dataType !== 'uuid') {
                    // We need to create a default UUID workout plan to link orphaned exercises
                    console.log("Creating a default workout plan with UUID...");
                    
                    // First check what column type workout_plans.id has
                    ...
                    if (workoutPlanIdType.length > 0 && workoutPlanIdType[0].data_type === 'uuid') {
                        // Get valid enum value for workoutCategory
                        ...
                        // Check if we already have a default workout plan
                        ...
                        if (defaultWorkout.length === 0) {
                            // Create a default workout plan with UUID
                        ...
                        // Create a new column with the UUID type
                        ...
                        // Link the existing workout_plans to the exercises using the UUID
                        console.log("Mapping integer IDs to UUIDs for workout plans...");
                        
                        // First check if we need to join through existing ids
                        ...
                        if (needsJoin[0].has_data) {
                            // Map existing integer IDs to UUIDs
                            await queryRunner.query(`
                        ...
                        if (parseInt(nullCount[0].count) > 0) {
                            // Update any remaining NULL references
                            await queryRunner.query(`
                        ...
                        // Drop indexes on workout_plan_id
                        ...
                        // Rename the UUID column to the original name
