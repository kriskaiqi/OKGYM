...
async function fixAchievementTable() {
  ...
  try {
    ...
    // Check if there are any invalid UUID entries
    logger.info("Finding records with invalid UUIDs...");
    ...
    if (invalidUUIDs && invalidUUIDs.length > 0) {
      logger.info(`Found ${invalidUUIDs.length} records with invalid UUIDs. Setting them to NULL...`);
      ...
      logger.info("No invalid UUIDs found.");
    }
