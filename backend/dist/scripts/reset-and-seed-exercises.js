...
async function resetAndSeedExercisesTable() {
    try {
        ...
        logger_1.default.info('Starting exercise seeding...');
        await (0, seedExercises_1.seedExercises)();
        const countFinal = await exerciseRepository.count();
        logger_1.default.info(`Final exercise count after seeding: ${countFinal}`);
        await data_source_1.AppDataSource.destroy();
        logger_1.default.info('Database connection closed');
    }
    catch (error) {
        logger_1.default.error('Error resetting and seeding exercises table:', error);
        if (data_source_1.AppDataSource.isInitialized) {
            await data_source_1.AppDataSource.destroy();
...
//# sourceMappingURL=reset-and-seed-exercises.js.map