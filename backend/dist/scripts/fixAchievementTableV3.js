...
async function fixAchievementTable() {
    ...
    `;
      ...
      `);
        ...
        logger_1.default.info("Finding records with invalid UUIDs...");
        const invalidUUIDsQuery = `
      SELECT id FROM achievement 
      WHERE workout_session_id IS NOT NULL 
      AND LENGTH(workout_session_id) < 36
    `;
        const invalidUUIDs = await queryRunner.query(invalidUUIDsQuery);
        if (invalidUUIDs && invalidUUIDs.length > 0) {
            logger_1.default.info(`Found ${invalidUUIDs.length} records with invalid UUIDs. Setting them to NULL...`);
            for (const record of invalidUUIDs) {
                await queryRunner.query(`
          UPDATE achievement 
        ...
        `, [record.id]);
            ...
            logger_1.default.info("No invalid UUIDs found.");
        }
...
//# sourceMappingURL=fixAchievementTableV3.js.map