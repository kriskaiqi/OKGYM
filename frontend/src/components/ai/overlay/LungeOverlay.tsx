...
import { LungeMetrics } from '../../../types/lunge';

interface LungeOverlayProps {
  metrics: LungeMetrics;
  errors: ExerciseError[];
  isLiveMode?: boolean;
}

const LungeOverlay: React.FC<LungeOverlayProps> = ({ 
  metrics, 
  errors,
  isLiveMode = false
}) => {
  ...
  const leftKneeAngle = metrics.leftKneeAngle || 0;
  const rightKneeAngle = metrics.rightKneeAngle || 0;
  ...
  const getBorderStyle = () => {
    // If there are errors, check the severity of the most critical one
    if (errors.length > 0) {
  ...
  // Calculate if knee angles are in acceptable range
  const isLeftKneeOK = leftKneeAngle >= 60 && leftKneeAngle <= 125;
  const isRightKneeOK = rightKneeAngle >= 60 && rightKneeAngle <= 125;
  ...
  return (
    <>
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: '15px', marginTop: '5px' }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <Typography sx={{ fontSize: '0.8rem', color: '#aaa' }}>LEFT</Typography>
            <Typography 
              sx={{ 
                fontSize: '1.8rem', 
                fontWeight: 'bold', 
                color: isLeftKneeOK ? '#4caf50' : '#f44336' 
              }}
            >
              {Math.round(leftKneeAngle)}°
            </Typography>
          </Box>
          <Box 
            sx={{ 
          ...
          />
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <Typography sx={{ fontSize: '0.8rem', color: '#aaa' }}>RIGHT</Typography>
            <Typography 
              sx={{ 
                fontSize: '1.8rem', 
                fontWeight: 'bold', 
                color: isRightKneeOK ? '#4caf50' : '#f44336' 
              }}
            >
              {Math.round(rightKneeAngle)}°
            </Typography>
          </Box>
        </Box>
      </Box>
