...
interface ShoulderPressOverlayProps {
  metrics: ShoulderPressMetrics;
  errors: ExerciseError[];
  isLiveMode?: boolean;
}

const ShoulderPressOverlay: React.FC<ShoulderPressOverlayProps> = ({ 
  metrics, 
  errors,
  isLiveMode = false
}) => {
  ...
  // Calculate arm angle difference and determine if they're even
  const armAngleDifference = Math.abs(leftArmAngle - rightArmAngle);
  const armsEven = armAngleDifference <= 15; // Consider arms even if within 15 degrees
  ...
  return (
    <>
      ...
      {/* Arm angles comparison - repositioned to avoid overlap with stage indicator */}
      <Box
        sx={{
          position: 'absolute',
          bottom: '100px',
...
export default ShoulderPressOverlay; 