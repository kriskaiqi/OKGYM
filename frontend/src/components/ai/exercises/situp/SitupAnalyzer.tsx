...
import { SitupMetricsDisplay } from './SitupMetricsDisplay';
...
declare global {
  interface Window {
    resetSitupCounter?: () => void;
  }
}

const SitupAnalyzer = forwardRef<{ resetCounter: () => void }, SitupAnalyzerProps>(
  ({ onAnalysisComplete, onError, isAnalyzing = true, targetReps = 10 }, ref) => {
    // State
    const [apiReady, setApiReady] = useState(false);
    const frameCounter = useRef<number>(0);
    const [showMetrics, setShowMetrics] = useState(true);
    const [currentMetrics, setCurrentMetrics] = useState<SitupMetrics>({});
    ...
    const connectAPI = useCallback(() => {
      ...
      try {
        logger.info('Setting up HTTP-based API connection for situp analysis');
        ...
        // Log success
        logger.info('API connection ready for situp analysis');
      } catch (error) {
    ...
    const resetCounter = useCallback(() => {
      try {
        fetch('http://localhost:3001/api/situp/reset', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ exerciseType: 'situp' })
        })
        .then(response => {
          if (!response.ok) {
        ...
        .then(data => {
          if (data.success) {
            logger.info('Successfully reset situp counter');
            ...
            logger.error('Failed to reset situp counter:', data.error);
          }
        })
        .catch(error => {
          logger.error('Error resetting situp counter:', error);
        });
      } catch (error) {
    ...
    useEffect(() => {
      ...
      window.resetSitupCounter = resetCounter;
    
      return () => {
        window.removeEventListener('workout_set_complete', resetCounter);
        delete window.resetSitupCounter;
      };
    }, [resetCounter]);
    ...
    const handleAnalysisResponse = useCallback((data: any) => {
      if (data.result) {
        ...
        if (newRepCount > lastRepCount) {
          // First rep - play form guidance for core exercise
          if (newRepCount === 1) {
            playExerciseTypeCue(ExerciseType.CORE, 0, targetReps);
          } 
        ...
        if (typedErrors.length > 0) {
          ...
          const currentErrorTypes = new Set<string>(typedErrors.map(e => e.type));
          ...
          if (newErrors.length > 0) {
            setTimeout(() => {
              ...
              if (newErrors.includes('straight_legs')) {
                playCue(AudioCueType.FULL_ROM); // "Full range of motion"
              } else if (newErrors.includes('incomplete_situp')) {
    ...
    const handleResults = useCallback((results: MediaPipeResults) => {
      ...
      try {
        ...
        // Use HTTP API
        fetch('http://localhost:3001/api/analyze/situp', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: payloadJson
        })
