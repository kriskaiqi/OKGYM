...
interface ShoulderPressMetricsDisplayProps {
  metrics?: ShoulderPressMetrics;
  errors?: Array<{type: string; message: string; severity: string}>;
}

export const ShoulderPressMetricsDisplay: React.FC<ShoulderPressMetricsDisplayProps> = ({ metrics = {}, errors = [] }) => {
  // Check for common errors
  const hasUnevenPressing = errors.some(error => error.type === 'uneven_pressing');
  const hasIncompletePress = errors.some(error => error.type === 'incorrect_form');
